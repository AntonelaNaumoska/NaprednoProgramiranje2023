...
char tocniVtori[4] = { 'c', 'c', 'a', 'a' };
char tocniTreti[4][2] = { {'a', 'c'}, {'a', 'd'}, {'b', 'd'}, {'a', 'c'} };
...
int main() {
    ...
    ifstream file_exist(filename);

    if (file_exist.good()) 
    {
        cout <<"\nYou have already taken the test! "<<endl;
        exit(0);
    }
    else 
    {
        ...
        string log_filename = "Students.log.txt";
        time_t now = time(nullptr);
        tm* timeinfo = localtime(&now);
        char vreme[80];
        strftime(vreme, sizeof(vreme), "%d.%m.%Y %H:%M:%S", timeinfo);
        ...
        ifstream log_file_read(log_filename,ios_base::app);
        ...
        log_file << count + 1 << ". " << vreme << "; " << lice.ime << "; " << lice.prezime << "; " << "INKI" << lice.indeks << "\n";
        ...
        file << "Vreme na start: " << vreme << endl;
        file.close();
